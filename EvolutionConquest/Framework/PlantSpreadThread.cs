...
public class PlantSpreadThread
{
    ...
    private void CheckForSpreadLoop()
    {
        while (true)
        {
            ...
            {
                System.IO.File.WriteAllText(System.IO.Path.Combine(_gameData.SessionID.ToString(), "ErrorLogPlantSpreadThread.txt"), "PLANTS Spread Thread Uncaught error: " + ex.Message + Environment.NewLine + "Stacktrace: " + ex.StackTrace);
            }
            Thread.Sleep(100);
        }
    }
